...
for (const f of files) {
  ...
  //
  // ★ 처음엔 "앞 절이 10자 이상" 이라는 하한을 두고 어미도 아홉 개만 봤다. 다섯 건을 놓쳤다.
  //     짬이냐, / 화면이라, / 때인데,   ← 계사(-이-) 활용형이 목록에 없었다
  //     재고,                          ← 명사 "재고" 로 오해해서 먼저 지워버렸다
  //     때, 몰래                       ← 어미가 아예 없는 쉼표
  //   그래서 하한을 없애고 어미를 넓혔다. 나열 쉼표(A, B, C)는 뒤에 조사 없이 명사가
  //   이어지므로 어미 검사에 안 걸린다.
  const NOUN_GO = /(냉장고|선고|사고|신고|광고|창고|참고|최고|중고|원고|경고|예고|금고|상고|항고), /g;
  const END = '고|며|는데|인데|지만|아서|어서|이라|이라서|면서|니까|라서|라|다가|여서|해서|되어|돼서|이냐|느냐|는지|을지';
  const JOIN = new RegExp(`([가-힣]{2,}(?:${END})),\\s`);
  const blocks = [...main.matchAll(/<(p|li|h[123]|blockquote)[^>]*>([\s\S]*?)<\/\1>/g)]
    .map((x) => strip(x[2]));
  const joins = blocks.filter((b) => JOIN.test(b.replace(NOUN_GO, ' ')));
  if (!legal && joins.length) {
    issues.push(`쉼표로 절 잇기 ${joins.length}건 — 첫 건: "${(joins[0].replace(NOUN_GO, ' ').match(JOIN) || ['', ''])[1]},"`);
  }
